#!/usr/bin/env python3
"""
Sistema de Gerenciamento Automático de Dados - Versão Completa com SQL Server
=============================================================================

Testa todos os três bancos de dados incluindo SQL Server
"""
...
try:
    ...
    print("   pip install pymysql psycopg2-binary pyodbc")
    print("   brew install unixodbc  # Para macOS")
    sys.exit(1)
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/Users/jonatasonca/Desktop/TecOnca/Projetos/databases-local/logs/auto-data-full-demo.log'),
...
class DataManager:
    ...
    def connect(self) -> bool:
        ...
        try:
            if self.database_type == 'mysql':
                ...
                # Para SQL Server, usar o banco master inicialmente e depois mudar para testdb
                conn_str = (
                    f"DRIVER={DatabaseConfig.SQLSERVER['driver']};"
                    f"SERVER={DatabaseConfig.SQLSERVER['server']};"
                    f"DATABASE=master;"
                    f"UID={DatabaseConfig.SQLSERVER['user']};"
                    f"PWD={DatabaseConfig.SQLSERVER['password']};"
                    f"TrustServerCertificate=yes"
                )
                self.connection = pyodbc.connect(conn_str)
                
                # Verificar/criar database testdb
                ...
                # Reconectar ao testdb
                conn_str = conn_str.replace("DATABASE=master", "DATABASE=testdb")
                self.connection = pyodbc.connect(conn_str)
    ...
    def update_produto(self) -> bool:
        ...
        if self.database_type == 'sqlserver':
            query = "UPDATE produtos SET preco = preco * ? WHERE id = ?"
        else:
    ...
    def insert_log(self) -> bool:
        ...
        if self.database_type == 'sqlserver':
            query = "INSERT INTO logs (mensagem) VALUES (?)"
        else:
...
def main():
    ...
    print(f"""
🗄️  Sistema de Gerenciamento Automático - DEMO COMPLETA
======================================================
